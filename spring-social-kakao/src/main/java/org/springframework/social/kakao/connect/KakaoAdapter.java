...
public class KakaoAdapter implements ApiAdapter<Kakao> {
	...
	public UserProfile fetchUserProfile(Kakao kakao) {
		KakaoProfile profile = kakao.userOperation().getUserProfile();
		return new UserProfileBuilder().setName(profile.getProperties().getNickname()).setUsername(profile.getProperties().getNickname()).setEmail(profile.getKaccount_email()).build();
	}
